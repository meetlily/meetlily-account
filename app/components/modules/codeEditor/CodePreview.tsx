...
export const CodePreview: FC<CodePreviewProps> = ({
	...
	if (code === '') {
		code = childrenList
			.map((child) => reactElementToJSXString(child, reactElementToJSXStringOptions))
			.join('\n');
	}
  ...
  return (
	...
	return (
		<div className="code-example">
			<div className="w-full rounded-none border border-gray-200 bg-gray-50 py-2 px-4 dark:border-gray-600 dark:bg-gray-700">
				<div className="grid grid-cols-2">
					<div className="mr-auto inline-flex items-center text-xl">Documentation</div>
					<EditOnGithubButton className="ml-auto" githubPage={githubPage} title={title} />
				...
				<div className="code-preview flex border-x border-gray-200 bg-white bg-gradient-to-r p-0 dark:border-gray-600 dark:bg-gray-900">
					<div className="code-responsive-wrapper w-full">
						<div className="mx-auto w-full bg-white bg-gradient-to-r p-2 dark:bg-gray-900 sm:p-6">
							<div className={twMerge('py-0', className)}>{children}</div>
						</div>
					</div>
				</div>
			</div>
			<div className="code-syntax-wrapper">
				<div className="code-syntax relative border-x border-y border-gray-200 dark:border-gray-600">
					...
					<pre
						className={twMerge('language-tsx !overflow-y-hidden p-5', !isExpanded && 'max-h-72')}
					>
						<code>{code}</code>
					</pre>
...
const ToggleDarkModeButton: FC<ComponentProps<'button'> & CodePreviewState> = ({
	isDarkMode,
	onClick
}) => {
	return (
...
const CopyToClipboardButton: FC<ComponentProps<'button'> & CodePreviewState> = ({
	isJustCopied,
	onClick
}) => {
	return (
...
const CollapseExpandButton: FC<ComponentProps<'button'> & CodePreviewState> = ({
	isExpanded,
	onClick
}) => {
	return (
...
const firstComponentDisplayName = (str: string) => {
	...
	return str
		.substring(1, firstSpace > firstClosingTag ? firstClosingTag : firstSpace)
		.replaceAll(/\s/g, '');
};
