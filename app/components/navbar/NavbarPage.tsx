...
const NavbarPage: React.FC<NavbarPageProps> = ({
	...
	return (
		<Navbar border={true} hidden={hideNavbar} className={classes} color={navColor}>
			...
			<div className={`flex md:order-${searchOrder}`}>{showSearch && <Search />}</div>
			<div className={`flex md:order-${ctaOrder}`}>
				{cta}
				<Navbar.Toggle />
			</div>
			<div className={`flex md:order-${itemsOrder}`}>
				<Navbar.Collapse>{items}</Navbar.Collapse>
			</div>
		</Navbar>
	);
};
