...
import { getSLSMembers } from '@/app/actions/getSLSData';
	...
	try {
		...
		if (currentUser && currentUser?.Role.length > 0) {
			...
			if (currentUser?.Role[0].name === 'Super Administrator') {
				...
				return new NextResponse('Your do not have permission to view Swinger members', {
					status: 401
				});
			}
		}
		return new NextResponse(
			'Your account does not have role assigned to it. Please contact system admin.',
			{ status: 500 }
		);
	} catch (error) {
